...
const PRODUTO_INICIAL = {
    ...
    descricao: '',
    imagem: null,
    imagemUrl: '' //exibe imagem existente
};

export default function FormCadProduto({ visible, onHide, onSave, produtoEdicao }) {


    const [produto, setProduto] = useState(PRODUTO_INICIAL);
    const [loading, setLoading] = useState(false); // Estado para indicar carregamento (útil para desabilitar botões ou mostrar spinner)
    ...
    useEffect(()=>{
        if(produtoEdicao){
            setProduto({
                ...
                imagem: produtoEdicao.imagem || null,
                imagemUrl: produtoEdicao.imagemUrl || ''
            })
        }
        else{
    ...
    const submit = async () => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:3001/api/produtos/${produtoEdicao.id}`, {
            method: 'PUT',
    ...
    const formularioValido = produto.nome && produto.preco && produto.categoria && produto.descricao && produto.imagem;

    return (
        <Dialog header="Editar produto" visible={visible} onHide={onHide}  modal
            footer={
